...
def __cell(fi, xml, id, share1="Share1", share2="Share2"):
    ...
    __write_prop(xml, buf, 0x02, 0x01, 0x11, "Uzor")
    ...
    __write_prop(xml, buf, 0x02, 0x02, 0x16, "UzorColor")
    ...
    __write_prop(xml, buf, 0x02, 0x08, 0x1b, "Type")
...
def __objs(fi, xml):
    ...
    if size1:
        ...
        for i in xrange(size1):
            ...
            #print [buf]
            tmp = unpack("=10i", buf)
            __write_element(xml,"objtype",str(tmp[0]))
            __write_element(xml,"countStart",str(tmp[1]))
            __write_element(xml,"pageStart",str(tmp[2]))
            __write_element(xml,"Xstart",str(tmp[3]))
            __write_element(xml,"YStart",str(tmp[4]))
            __write_element(xml,"quant",str(tmp[5]))
            __write_element(xml,"pageend",str(tmp[6]))
            __write_element(xml,"xend",str(tmp[7]))
            __write_element(xml,"yend",str(tmp[8]))
            __write_element(xml,"level",str(tmp[9]))
            if tmp[0]==5:
                # рисунок
...
mxl2xml("D:\OpenServer\domains\localhost\gridEditor\converter\\1.mxl")
